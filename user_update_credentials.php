...
$user_info = mysqli_fetch_assoc($user_results);
...
if (isset($_GET['verification_flag'])) {
    ...
    // echo $_GET['verification_flag'] . '<br>';
    // echo $verification_flag . '<br>';
}

if (isset($_POST['old_password']) && isset($_POST['password']) && isset($_POST['re_password'])) {
    $verification = "select * from login where email = '" . $login . "'";
    ...
    if ($verification_flag == "1") {
        $registration_query = "UPDATE `login` SET `password` = '" . $_POST['password'] . "' WHERE `login`.`email` = '" . $login . "';";
        $result = mysqli_query($conn, $registration_query);
        if ($result) {
            $registration_status = 'Password Changed!';
            header('Location: ' . $_SERVER['PHP_SELF'] . '?login=' . $login . '&verification_flag=' .$verification_flag);
        }
    }
}
        ...
        <form action="" role='register' method='post'>
            ...
            <div class="input-group mb-3">
                <div class="form-floating mb-3">
                    <input type="text" class="form-control" value="<?php echo $user_info['name']; ?>" id="floatingInputDisabled" placeholder="name@example.com" disabled>
            ...
            <div class="input-group mb-3">
                <div class="form-floating mb-3">
                    <input type="text" class="form-control" value="<?php echo $user_info['email']; ?>" id="floatingInputDisabled" placeholder="name@example.com" disabled>
